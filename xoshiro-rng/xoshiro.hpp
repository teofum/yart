...
static inline std::uint32_t extract_32bits_from64_right(const std::uint64_t x) {
  ...
  std::memcpy(
    &out,
    reinterpret_cast<const std::uint32_t*>(&x) + 1,
    sizeof(std::uint32_t));
  return out;
}

static inline void assign_32bits_to64_left(
  std::uint64_t assign_to,
  const std::uint32_t take_from
) {
  std::memcpy(
    reinterpret_cast<std::uint32_t*>(&assign_to),
    &take_from,
    sizeof(std::uint32_t));
}

static inline void assign_32bits_to64_right(
  std::uint64_t assign_to,
  const std::uint32_t take_from
) {
  std::memcpy(
    reinterpret_cast<std::uint32_t*>(&assign_to) + 1,
    &take_from,
    sizeof(std::uint32_t));
}

constexpr static const uint64_t JUMP_X256PP[] = {0x180ec6d33cfd0aba,
                                                 0xd5a61266f0c9392c,
                                                 0xa9582618e03fc9aa,
                                                 0x39abdc4529b1661c};

constexpr static const uint64_t LONG_JUMP_X256PP[] = {0x76e15d3efefdcbbf,
                                                      0xc5004e441c522fb3,
                                                      0x77710069854ee241,
                                                      0x39109bb02acbe635};

constexpr static const uint32_t JUMP_X128PP[] = {0x8764000b, 0xf542d2d3,
                                                 0x6fa035c3, 0x77f2db5b};

constexpr static const uint32_t LONG_JUMP_X128PP[] = {0xb523952e, 0x0b6f099f,
                                                      0xccf5a0ef, 0x1c580662};
...
public:
  ...
  std::uint64_t state[4] = {0x3d23dce41c588f8c, 0x10c770bb8da027b0,
                            0xc7a4c5e87c63ba25, 0xa830f83239465a2e};
  ...
  void seed(Sseq& seq) {
    seq.generate(
      reinterpret_cast<std::uint32_t*>(&this->state[0]),
  ...
  result_type operator()() {
    const std::uint64_t result =
      rotl64(this->state[0] + this->state[3], 23) + this->state[0];
  ...
  operator<<(std::basic_ostream<CharT, Traits>& ost, const Xoshiro256PP& e) {
    ost
      .write(reinterpret_cast<const char*>(&e.state[0]), sizeof(std::uint64_t));
    ost.put(' ');
    ost
      .write(reinterpret_cast<const char*>(&e.state[1]), sizeof(std::uint64_t));
    ost.put(' ');
    ost
      .write(reinterpret_cast<const char*>(&e.state[2]), sizeof(std::uint64_t));
    ost.put(' ');
    ost
      .write(reinterpret_cast<const char*>(&e.state[3]), sizeof(std::uint64_t));
    return ost;
  }
...
public:
  ...
  result_type operator()() {
    const std::uint32_t result =
      rotl32(this->state[0] + this->state[3], 7) + this->state[0];
  ...
  operator<<(std::basic_ostream<CharT, Traits>& ost, const Xoshiro128PP& e) {
    ost
      .write(reinterpret_cast<const char*>(&e.state[0]), sizeof(std::uint32_t));
    ost.put(' ');
    ost
      .write(reinterpret_cast<const char*>(&e.state[1]), sizeof(std::uint32_t));
    ost.put(' ');
    ost
      .write(reinterpret_cast<const char*>(&e.state[2]), sizeof(std::uint32_t));
    ost.put(' ');
    ost
      .write(reinterpret_cast<const char*>(&e.state[3]), sizeof(std::uint32_t));
    return ost;
  }
