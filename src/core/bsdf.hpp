...
class GGX {
public:
  constexpr explicit GGX(float roughness) noexcept: m_roughness(roughness) {
  ...
  [[nodiscard]] constexpr float r() const noexcept {
    return m_roughness;
  }
...
private:
  float m_alphaX = 0.0f, m_alphaY = 0.0f, m_roughness = 0.0f;
